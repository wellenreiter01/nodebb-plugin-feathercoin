...
var walletAddress = $.get(RELATIVE_PATH + '/api/crypto', {}, function(addressData) {
    ...
    if(addressData.address){
    	...
    	var socket = io("https://explorer.feathercoin.com:443/");
		...
		socket.on(addressData.address, function(data) {
			...
			$.get("https://explorer.feathercoin.com:443/api/tx/" + data, function(transDetails,exp) {
				...
				app.alert({
					title: 'Tip Recieved',
					message: 'You received a tip of ' + transDetails.vout[vOut].value + ' FTC from : ' + transDetails.vin[0].addr, //search for user name from address
					location: 'right-top',
				/*	timeout: 15000, */
					type: 'success'
				});
